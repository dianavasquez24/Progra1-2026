...
float CalcularPrecioTotal(float precioBase, float impuesto = 13); // definimos impuestos una vez dentro del prototipo
...
float CalcularPrecioTotal(float precioBase, float impuesto) {
    double total;
    total = precioBase + (precioBase * impuesto / 100); // 13 %
    return total;
}